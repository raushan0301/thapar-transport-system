...
const AuthorityDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        const { data: requests, error } = await supabase.from('transport_requests').select('*').eq('routed_authority_id', user.id).order('submitted_at', { ascending: false });
        if (error) throw error;

        const { data: myApprovals } = await supabase.from('approvals').select('request_id, action').eq('approver_id', user.id).eq('approver_role', profile.role);
        ...
        requests.forEach(r => {
          ...
          if (r.current_status === 'pending_authority') pending++;
        ...
        setRecentRequests(requests.filter(r => r.current_status === 'pending_authority').slice(0, 5));
      } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <DashboardLayout>
        <div className="mb-8 animate-slideDown">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Authority Dashboard</h1>
          <p className="text-gray-600">Review and approve transport requests routed to you</p>
        </div>
            ...
            <div className="relative preserve-3d transition-all duration-500 hover:rotate-y-3">
              ...
              <div className="relative bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 hover:border-amber-200 transform hover:-translate-y-1">
                <div className="flex justify-between items-center mb-6">
                  ...
                  {stats.pending > 0 && <Button variant="ghost" size="sm" onClick={() => navigate('/authority/pending')} className="text-amber-600 hover:text-amber-700 hover:bg-amber-50">View All <ArrowRight className="w-4 h-4 ml-1" /></Button>}
                </div>
          ...
          <div className="group perspective-1000" style={{ animation: 'slideUp 0.6s ease-out forwards', animationDelay: '500ms', opacity: 0 }}>
            <div className="relative preserve-3d transition-all duration-500 hover:rotate-y-3">
              ...
              <div className="relative bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 hover:border-blue-200 transform hover:-translate-y-1">
                ...
                <div className="space-y-3">
                  <button className="w-full text-left px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-lg transition-all duration-200 border border-transparent hover:border-gray-200 group/btn" onClick={() => navigate('/authority/pending')}>
                    <div className="flex items-center justify-between">
                  ...
                  <button className="w-full text-left px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-lg transition-all duration-200 border border-transparent hover:border-gray-200 group/btn" onClick={() => navigate('/authority/approved')}>
                    <div className="flex items-center justify-between">
