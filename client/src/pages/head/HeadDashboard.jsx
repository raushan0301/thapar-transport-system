  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const { data: myApprovals, error: approvalsError } = await supabase
          .from('approvals')
          .select('request_id, action')
          .eq('approver_id', user.id)
          .eq('approver_role', 'head');
        ...
        if (myApprovals) {
          myApprovals.forEach(approval => {
            approvalMap[approval.request_id] = approval.action;
          });
        }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <DashboardLayout>
        <div className="mb-8 animate-slideDown">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Head Dashboard</h1>
          <p className="text-gray-600">Review and approve transport requests from your department</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {statsData.map((stat, index) => (
            <div key={index} className="group perspective-1000" style={{ animation: 'slideUp 0.6s ease-out forwards', animationDelay: `${index * 100}ms`, opacity: 0 }}>
              <div className="relative preserve-3d transition-all duration-500 hover:rotate-y-6 hover:rotate-x-3">
                ...
                <div className="relative bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 hover:border-gray-200 transform hover:-translate-y-2">
                  <div className="mb-4">
                    <stat.icon className={`w-12 h-12 transform transition-transform duration-500 group-hover:scale-110 ${
                  ...
                  <div className="mb-2">
                    <p className="text-4xl font-bold text-gray-900 transition-all duration-500 group-hover:scale-110 origin-left">{stat.value}</p>
                  </div>
            ...
            <div className="relative preserve-3d transition-all duration-500 hover:rotate-y-3">
              ...
              <div className="relative bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 hover:border-amber-200 transform hover:-translate-y-1">
                <div className="flex justify-between items-center mb-6">
                  ...
                  {stats.pending > 0 && (
                    <Button variant="ghost" size="sm" onClick={() => navigate('/head/pending')} className="text-amber-600 hover:text-amber-700 hover:bg-amber-50">
          ...
          <div className="group perspective-1000" style={{ animation: 'slideUp 0.6s ease-out forwards', animationDelay: '500ms', opacity: 0 }}>
            <div className="relative preserve-3d transition-all duration-500 hover:rotate-y-3">
              ...
              <div className="relative bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 hover:border-blue-200 transform hover:-translate-y-1">
                ...
                <div className="space-y-3">
                  <button className="w-full text-left px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-lg transition-all duration-200 border border-transparent hover:border-gray-200 group/btn" onClick={() => navigate('/head/pending')}>
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-semibold text-gray-900">Review Requests</p>
                        <p className="text-sm text-gray-600">{stats.pending} pending approval</p>
                      </div>
